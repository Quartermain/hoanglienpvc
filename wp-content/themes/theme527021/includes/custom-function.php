				...
				while ( $carousel_query->have_posts() ) : $carousel_query->the_post();
					...
					$output .= '<div class="item ' . $format . ' item-list-'.$itemcounter.'">';
						...
						$output .= '<div class="desc">';
							...
							// post date
							$output .= $date ? '<time datetime="' . get_the_time( 'Y-m-d\TH:i:s', $post_id ) . '">' . get_the_date('d') .'/ <span>'. get_the_date('m') . '</span></time>' : '';

							// post author
							$output .= $author ? '<em class="author">&nbsp;<span>' . __('by ', CURRENT_THEME) . '</span>&nbsp;<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ).'">' . get_the_author_meta( 'display_name' ) . '</a> </em>' : '';
							...
							// post excerpt
							if ( !empty($excerpt{0}) ) {
								$output .= $excerpt_count > 0 ? '<p class="excerpt"><a href="' . $post_permalink . '">' . wp_trim_words( $excerpt, $excerpt_count ) . '</a></p>' : '';
							}
			...
			if ($post_tags && !is_wp_error($post_tags)) {
				...
				if ( have_posts() ) {
					...
					while( have_posts() ) {
						...
						$output .= $display_link ? '<h4><a href="'.get_permalink().'" >'.get_the_title().'</a></h4>': '' ;
						$output .= '</li>';
					}
					$output .= '</ul></div>';
					echo $output;
